...
post_fitted_mean <- post_fitted_recode %>% group_by(measured_variant, recent_exposure, infection_history) %>%
...
post_fitted_mean_cens_dist <- post_fitted_mean %>% mutate(diff = abs_titre - abs_titre_censored) %>% filter(diff > 0)
...
p2 <- pred_post_plot_clean %>% filter(t < 150) %>%
    ggplot() +
        geom_hline(yintercept = 5 * 2^7, linetype = "dashed", color = "gray30") + 
        geom_hline(yintercept = 5 * 2^1, linetype = "dashed", color = "gray30") + 
        geom_line(aes(x = t, y = 5 * 2^fitted_val, color = recent_exposure, linetype = infection_history), size = 1, alpha = 0.8) + 
...
p3 <- post_fitted_mean %>% 
    ggplot() + 
        ...
        geom_segment(data = post_fitted_mean_cens_dist,
            aes(x =  2^wane_s, y = 5 * 2^abs_titre_censored, xend =  2^wane_s, yend = 5 * 2^(abs_titre)),  
            size = 0.5, alpha = 0.8, linetype = "dashed",
            color = "gray15") +
        geom_point(data = post_fitted_mean_cens_dist,
            aes(x =  2^wane_s, y = 5 * 2^abs_titre),  
            size = 5, alpha = 0.8, shape = 4,
            color = "gray15") +
        geom_point(aes(x =  2^wane_s, y = 5 * 2^abs_titre_censored, fill = recent_exposure, shape = infection_history), size = 4, alpha = 0.8, 
            color = "black") +
        facet_grid(cols = vars(measured_variant), rows = vars(number_ag_exposure)) + 
        ...
        scale_y_continuous(trans = "log2", limits = c(10, 2000), breaks = 2 ^ (1:7) * 5, labels = labs_plot_y) + scale_x_continuous(trans = "log2", breaks = 2 ^ seq(-7, 1, 0.5), labels = round(2 ^ seq(-7, 1, 0.5), 2 ), limits = c(0.3, 1) ) +
        guides(color = "none") +
...
ggsave(here::here("outputs", "figs", "final_figs", "fig1.png"), height = 16, width = 12)
...
p2 <- pred_post_plot_clean %>% mutate(fitted_val_boost = fitted_val - min(fitted_val)) %>% filter(t < 150) %>%
    ggplot() +
        geom_line(aes(x = t, y = 2^fitted_val_boost, color = recent_exposure, linetype = infection_history), size = 1, alpha = 0.8) + 
        facet_grid(cols = vars(measured_variant), rows = vars(number_ag_exposure)) + 
      #  scale_color_manual(values = c("red", "darkred", "blue", "darkblue", "lightgreen", "green")) +
        scale_linetype_manual(values = c("dashed", "solid")) +
        scale_y_continuous(trans = "log2", limits = c(1, 32), breaks = 2 ^ (0:7), labels = labs_plot_y_gtr) + 
        theme_bw() +
        guides(linetype = guide_legend(override.aes = list(size = c(0.5, 0.5)))) +
        theme(text = element_text(size = 15)) +
        scale_color_manual(values = colorsdefined) +
...
p3 <- post_fitted_mean %>% 
    ggplot() + 
        geom_point(aes(x =  2^wane_s, y = 2^boost, fill = recent_exposure, shape = infection_history), size = 4, alpha = 0.8, 
            color = "black") +
        facet_grid(cols = vars(measured_variant), rows = vars(number_ag_exposure)) + 
        scale_shape_manual(values = c("Infection naive" = 21, "Previously infected" = 22)) +
        guides(fill = guide_legend(override.aes = list(shape = c(21, 21, 21)))) +
...
require(patchwork)
p1 / p2 / p3  + plot_layout(heights = c(1, 2, 2)) + plot_annotation(tag_levels = "A") 
ggsave(here::here("outputs", "figs", "final_figs", "figS1.png"), height = 16, width = 12)
